...
//      surfaceScalarField Eave = fvc:: interpolate(rhoE/rho) ;
...
//	with RANS and turbulence model
	muEff = turbulence -> muEff() ;
        muEff += rho * 2.0 * mag((rho-rhoFil)/(rho+rhoFil)) * Chi * Foam::pow(cellVolu,2.0/3.0) *  Foam::sqrt(2.0)*mag(0.5*symm(fvc::grad(U))) ;
...
//	Classical flux on the boundaries

//      enFlux.boundaryFieldRef() = rhoave.boundaryField()*Have.boundaryField()*phi.boundaryField() ;
//      enFlux = enFlux - enVisFlux        ;
